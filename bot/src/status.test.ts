...
describe('statusEmbed', () => {
	test('shows players, map, coloured scores and the leader colour', () => {
		const e = statusEmbed({ name: 'EU', status, error: '' }, now, 60);
		expect(e.title).toBe('WD #1');
		expect(e.description).toBe('**12 / 98** players on **Bakurani**');
		expect(e.color).toBe(0xd86060);
		expect(e.fields?.[0]?.value).toBe('\u{1F7E5} Rebels **40**\n\u{1F7E6} Army **25**');
		expect(e.fields?.[1]?.value).toBe('1 h 5 min');
		expect(e.fields?.[2]?.value).toBe('100');
	...
	test('a tie is grey and no cap reads none', () => {
		...
		const e = statusEmbed({ name: 'EU', status: tied, error: '' }, now, 90);
	...
	test('offline shows the error in red', () => {
		const e = statusEmbed({ name: 'EU', status: null, error: 'Could not reach x' }, now, 60);
